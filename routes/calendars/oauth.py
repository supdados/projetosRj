...
def start_google_calendar_oauth():
    if not is_google_calendar_enabled(current_app.config):
        flash(
            "Integração Google Calendar indisponível. Verifique client_secret e variáveis de ambiente.",
            "warning",
        )
...
def google_calendar_oauth_callback():
    ...
    redirect_uri = (
        session.pop(GOOGLE_AUTH_REDIRECT_SESSION_KEY, None)
        or _resolve_runtime_google_redirect_uri()
    )
    ...
    if sync_issue or watch_issue or identity_issue:
        ...
        if identity_issue:
            flash(
                f"Conta Google conectada sem identificação completa: {identity_issue}",
                "warning",
            )
        if sync_issue:

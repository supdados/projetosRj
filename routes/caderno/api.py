...
def _next_position(user_id):
    max_pos = (
        db.session.query(db.func.max(CadernoBlock.position))
        .filter(CadernoBlock.user_id == user_id)
        .scalar()
    )
...
def _build_layout_payload(
    data, *, block_type="text", current_block=None, for_create=False
):
    resolved_block_type = (
        current_block.block_type if current_block is not None else block_type
    )
    defaults = _default_layout_for_block_type(resolved_block_type)

    grid_w = _clamp_grid_width(
        data.get(
            "grid_w", current_block.grid_w if current_block else defaults["grid_w"]
        ),
    ...
    grid_h = _clamp_grid_height(
        data.get(
            "grid_h", current_block.grid_h if current_block else defaults["grid_h"]
        ),
        block_type=resolved_block_type,
    ...
    if current_block is None and (grid_x is None or grid_y is None):
        ...
        next_grid_x, next_grid_y = _find_next_available_slot(
            existing_blocks, grid_w, grid_h
        )
...
def _serialize_sheet(state):
    ...
    if state is not None:
        expand_steps = _coerce_int(
            state.expand_steps, default=0, minimum=0, maximum=MAX_CADERNO_EXPAND_STEPS
        )
    return {
...
def _serialize_block(block):
    base = {
        ...
        "attached_to_block_id": _coerce_int(
            block.attached_to_block_id, default=None, minimum=1
        ),
    ...
    if block.block_type == "project" and block.reference_id:
        ...
        if proj:
            base["ref_data"] = {
                ...
                "etapas_concluidas": sum(
                    1 for etapa in proj.workflow_etapas if etapa.done
                ),
            }
        ...
        if etapa:
            base["ref_data"] = {
                ...
                "data_inicio": (
                    etapa.data_inicio.isoformat() if etapa.data_inicio else None
                ),
        ...
        if task:
            base["ref_data"] = {
                ...
                "project_titulo": (
                    task.project.titulo if task.project else "Sem projeto"
                ),
...
def caderno_api_blocks():
    blocks = (
        CadernoBlock.query.filter(CadernoBlock.user_id == g.user.id)
        .order_by(
            CadernoBlock.grid_y.asc(), CadernoBlock.grid_x.asc(), CadernoBlock.id.asc()
        )
        .all()
    )
    state = CadernoState.query.filter_by(user_id=g.user.id).first()
...
def caderno_api_blocks_create():
    ...
    if block_type in ("project", "etapa", "tarefa"):
        ...
        if not reference_id or not isinstance(reference_id, int) or reference_id <= 0:
            return (
                jsonify({"error": "reference_id obrigatório para este tipo de bloco."}),
                400,
            )
    ...
    db.session.commit()
    return (
        jsonify({"block": _serialize_block(block), "sheet": _serialize_sheet(None)}),
        201,
    )
...
def caderno_api_state_update():
    ...
    if (
        expand_steps is None
        or expand_steps < 0
        or expand_steps > MAX_CADERNO_EXPAND_STEPS
    ):
        return (
            jsonify(
                {
                    "error": f"expand_steps deve estar entre 0 e {MAX_CADERNO_EXPAND_STEPS}."
                }
            ),
            400,
        )
...
def caderno_api_block_update(block_id):
    ...
    if {"attached_to_block_id", "attached_offset_x", "attached_offset_y"} & set(
        data.keys()
    ):

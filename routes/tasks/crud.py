...
def _check_restricted_fields_permission(task, inputs):
    ...
    responsavel_changed = _normalize_responsavel_value(
        inputs.responsavel_raw
    ...
    return (
        jsonify({"success": False, "message": EDIT_RESTRICTED_FIELDS_DENIED_MESSAGE}),
        403,
    )
...
def reorder_tasks_hub():
    ...
    if not ordem_ids:
        return jsonify(
            {"success": True, "message": "Nenhuma alteração de ordem enviada"}
        )
...
def edit_task(task_id):
    ...
    project, project_error_response = _resolve_project_for_edit(
        inputs.project_raw, task
    )
    ...
    if not _can_transition_task_to_status(
        g.user, task, inputs.status, previous_status=task.status
    ):
        _audit_denied_task_action(
            task, "forbidden_finalize", attempted_status=inputs.status
        )
...
def delete_task(task_id):
    ...
    try:
        ...
        return _on_db_error_redirect(
            e,
            is_ajax=is_ajax,
            error_prefix="Erro ao excluir tarefa",
            endpoint="main.list_tasks",
            item_id=task_id,
        )
...
def update_task_status(task_id):
    ...
    if not _can_transition_task_to_status(
        g.user, task, status, previous_status=task.status
    ):
...
def update_task_prioridade(task_id):
    ...
    if not _can_manage_task_restricted_actions(g.user, task):
        ...
        return (
            jsonify(
                {"success": False, "message": EDIT_RESTRICTED_FIELDS_DENIED_MESSAGE}
            ),
            403,
        )
...
def finalize_task(task_id):
    ...
    if task.is_archived:
        if is_ajax:
            return (
                jsonify(
                    {"success": False, "message": "Esta tarefa já está arquivada."}
                ),
                400,
            )
    ...
    try:
        ...
        return _on_db_error_redirect(
            e,
            is_ajax=is_ajax,
            error_prefix="Erro ao finalizar tarefa",
            endpoint="main.list_tasks",
        )
...
def unarchive_task(task_id):
    ...
    try:
        ...
        return _on_db_error_redirect(
            e,
            is_ajax=is_ajax,
            error_prefix="Erro ao desarquivar tarefa",
            endpoint="main.list_tasks_archived",
        )
...
def archive_finalized_tasks():
    ...
    try:
        ...
        return _on_db_error_redirect(
            e,
            is_ajax=is_ajax,
            error_prefix="Erro ao arquivar tarefas",
            endpoint="main.list_tasks",
        )
...
def get_hub_assignable_users():
    ...
    if project_raw:
        ...
        orgao_id, invalid_orgao_filter = sanitize_orgao_filter_for_current_user(
            resolved_orgao_raw
        )
        if invalid_orgao_filter:
            return (
                jsonify(
                    {"success": False, "message": "Sem permissão para este órgão."}
                ),
                403,
            )
        ...
        return (
            jsonify({"success": False, "message": "Informe um projeto ou órgão."}),
            400,
        )

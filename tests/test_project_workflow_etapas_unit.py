...
def _etapa(
    project, *, descricao, ordem=0, iniciada=False, done=False, entry_type="manual"
):
    etapa = Etapa(
        descricao=descricao,
...
def test_workflow_etapas_filters_out_google_meetings(app):
    with app.app_context():
        ...
        _etapa(
            project, descricao="Reunião kickoff", ordem=1, entry_type="google_meeting"
        )

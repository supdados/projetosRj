...
def test_add_etapa_inline_ajax_requires_confirmation_for_finalized_project(app, client_user, seed_data):
    with app.app_context():
...
def test_add_etapa_inline_ajax_reactivates_project_when_confirmation_is_sent(app, client_user, seed_data):
    with app.app_context():
        project = db.session.get(Project, seed_data['project_complete_id'])
        project.status = 'Finalizado'
        db.session.commit()

...
def test_move_orgao_json_happy_path(client_admin, app, seed_data):
    ...
    with app.app_context():
        alt = OrgaoUnidade(
            nome='Secretaria Alternativa', sigla='SALT', tipo='Secretaria',
            pai_id=seed_data['orgao_root_id'], ordem=1, ativo=True,
        )
        db.session.add(alt)
        db.session.commit()
        alt_id = alt.id
    ...
    with app.app_context():
        sub = OrgaoUnidade(
            nome='Subsecretaria Movel', sigla='SUMO', tipo='Subsecretaria',
...
def test_delete_blocked_when_has_children(client_admin, app, seed_data):
    ...
    with app.app_context():
        sub = OrgaoUnidade(
            nome='Sub Temporaria', sigla='STMP', tipo='Subsecretaria',
            pai_id=seed_data['orgao_child_id'], ordem=0, ativo=True,
        )
        db.session.add(sub)
        db.session.commit()
...
def test_reorder_swaps_siblings(client_admin, app, seed_data):
    with app.app_context():
        second = OrgaoUnidade(
            nome='Secretaria B', sigla='SECB', tipo='Secretaria',
            pai_id=seed_data['orgao_root_id'], ordem=1, ativo=True,
        )

...
def test_login_govbr_callback_rejects_invalid_state(app, client):
    ...
    response = client.get('/auth/govbr/callback?code=abc&state=other-state', follow_redirects=False)
...
def test_login_govbr_callback_success_with_user_mapped_by_cpf(app, client, seed_data, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_routes,
        'fetch_userinfo',
        lambda _config, *, access_token: {'preferred_username': '12345678901', 'sub': 'sub-1'},
    )
    ...
    response = client.get('/auth/govbr/callback?code=ok&state=state-1', follow_redirects=False)
...
def test_login_govbr_callback_fallback_by_username_sets_cpf(app, client, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_routes,
        'fetch_userinfo',
        lambda _config, *, access_token: {'preferred_username': '12345678901', 'sub': 'sub-2'},
    )
    ...
    response = client.get('/auth/govbr/callback?code=ok&state=state-2', follow_redirects=False)
...
def test_login_govbr_callback_blocks_when_user_is_not_linked(app, client, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_routes,
        'fetch_userinfo',
        lambda _config, *, access_token: {'preferred_username': '11122233344', 'sub': 'sub-3'},
    )
    ...
    response = client.get('/auth/govbr/callback?code=ok&state=state-3', follow_redirects=False)
...
def test_login_govbr_callback_blocks_when_sub_conflicts(app, client, seed_data, monkeypatch):
    _enable_govbr(app)
    ...
    monkeypatch.setattr(
        auth_routes,
        'fetch_userinfo',
        lambda _config, *, access_token: {'preferred_username': '12345678901', 'sub': 'sub-new'},
    )
    ...
    response = client.get('/auth/govbr/callback?code=ok&state=state-4', follow_redirects=False)
...
def test_logout_for_govbr_session_redirects_to_federated_when_enabled(app, client, seed_data, monkeypatch):
    _enable_govbr(app)
...
def test_logout_for_local_session_keeps_regular_redirect(app, client, seed_data, monkeypatch):
    _enable_govbr(app)
    monkeypatch.setattr(
        auth_routes,
        'build_logout_url',

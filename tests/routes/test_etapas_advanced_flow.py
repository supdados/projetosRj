...
def test_import_model_creates_stages_with_sequential_dates_and_history(app, client_user, seed_data):
    ...
    with app.app_context():
        etapas = (
            Etapa.query
            .filter_by(project_id=seed_data['project_id'])
        ...
        history = (
            ...
            .filter_by(project_id=seed_data['project_id'], action_type='import_model')
            .order_by(ProjectHistory.id.desc())
            .first()
        )
        assert history is not None
        assert 'Importou 2 etapa(s) do modelo "Template Base"' in history.action_description
...
def test_toggle_iniciada_clears_done_when_stage_is_reopened(app, client_user, seed_data):
    with app.app_context():
    ...
    response = client_user.post(f"/etapa/{seed_data['etapa_started_id']}/toggle_iniciada")
...
def test_toggle_done_requires_started_stage_and_toggles_successfully(client_user, seed_data):
...
def test_update_etapa_comentario_can_save_and_remove_comment(app, client_user, seed_data):

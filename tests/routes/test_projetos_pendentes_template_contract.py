...
def test_projetos_pendentes_shows_orgao_filter_for_non_admin_with_multiple_orgaos(
    ...
    response = client.get(
        "/projetos_pendentes", query_string={"orgao": str(vpd_orgao_id)}
    )
    assert response.status_code == 200
    html = response.get_data(as_text=True)
...
def test_projetos_pendentes_redirects_when_non_admin_forces_foreign_area(

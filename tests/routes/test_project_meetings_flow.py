...
import routes.etapas as etapa_routes
...
def _seed_project_meeting(seed_data, *, owner_user_id, owner_account_id, owner_email, sync_status='ok'):
...
def test_add_project_meeting_ajax_success_creates_shared_stage_and_link(app, client_user, seed_data, monkeypatch):
    ...
    def fake_sync(_config, event, _connection, create_conference=False):
        ...
        event.meet_link = 'https://meet.google.com/aaa-bbbb-ccc' if create_conference else None
        return event
    ...
    with app.app_context():
        ...
        history = (
            ...
            .filter_by(project_id=seed_data['project_id'], action_type='add_google_meeting')
...
def test_same_google_account_on_another_internal_user_can_reschedule_project_meeting(app, client, seed_data, monkeypatch):
    with app.app_context():
    ...
    monkeypatch.setattr(etapa_routes, 'sync_local_event_to_google', lambda *_args, **_kwargs: _args[1])

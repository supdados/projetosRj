...
def test_add_etapa_html_redirects_back_with_success_flash_and_persists_stage(app, client_user, seed_data):
    ...
    with app.app_context():
        etapa = (
            Etapa.query
            .filter_by(project_id=seed_data['project_id'], descricao='Nova etapa HTML')
...
def test_add_etapa_html_requires_confirmation_for_finalized_project(app, client_user, seed_data):
    with app.app_context():
    ...
    html = _follow_redirect_and_get_html(client_user, response)
    assert 'Ao adicionar uma nova etapa, o projeto voltará para o status Vigente. Deseja continuar?' in html

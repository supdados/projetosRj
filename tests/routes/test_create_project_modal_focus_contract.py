...
def test_create_project_buttons_use_focus_visible_instead_of_focus_hover_lock():
    ...
    dashboard_css = (PROJECT_ROOT / "static" / "css" / "index.css").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        ".dashboard-page-v2 .glass-btn.dashboard-new-project-btn:focus-visible"
        in dashboard_css
    )
    assert (
        ".dashboard-page-v2 .glass-btn.dashboard-new-project-btn:focus,"
        not in dashboard_css
    )
    assert (
        ".dashboard-page-v2 .dashboard-new-project-btn:focus-visible" in dashboard_css
    )

...
def test_api_templates_exposes_stage_count_and_total_duration(app, client_admin):
    with app.app_context():
        ...
        db.session.add(tpl)
        db.session.flush()
        db.session.add(StageTemplateItem(name='A', duration_days=3, order=0, templateId=tpl.id))
        db.session.add(StageTemplateItem(name='B', duration_days=5, order=1, templateId=tpl.id))
        db.session.add(StageTemplateItem(name='C', duration_days=2, order=2, templateId=tpl.id))
        db.session.commit()
        tpl_id = tpl.id
...
def test_admin_can_duplicate_template(app, client_admin, seed_data):
    ...
    with app.app_context():
        ...
        original_items = StageTemplateItem.query.filter_by(templateId=original_id).order_by(StageTemplateItem.order).all()
        copy_items = StageTemplateItem.query.filter_by(templateId=copy.id).order_by(StageTemplateItem.order).all()
        assert [(i.name, i.duration_days, i.order) for i in copy_items] == [
            (i.name, i.duration_days, i.order) for i in original_items
        ]


def test_admin_template_edit_preserves_getlist_order_with_five_stages(app, client_admin, seed_data):
    ...
    with app.app_context():
        items = StageTemplateItem.query.filter_by(
            templateId=seed_data['template_id']
        ).order_by(StageTemplateItem.order.asc()).all()
...
def test_admin_templates_list_filters_by_query(client_admin, app):
    with app.app_context():
        db.session.add(StageTemplate(name='Aquisição Direta', description='Para compras rápidas'))
        db.session.add(StageTemplateItem(
            name='Etapa Única',
            duration_days=1,
            order=0,
            templateId=StageTemplate.query.filter_by(name='Aquisição Direta').first().id,
        ))
        db.session.commit()
...
def test_admin_templates_list_paginates_at_page_size(client_admin, app):
    with app.app_context():
        ...
        for i in range(10):
            ...
            db.session.flush()
            db.session.add(StageTemplateItem(
                name='X', duration_days=1, order=0, templateId=tpl.id,
            ))
        db.session.commit()

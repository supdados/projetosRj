...
def test_caderno_drag_measurement_avoids_extra_buffer_rows():
    file_path = (
        Path(__file__).resolve().parents[2] / "static" / "js" / "pages" / "caderno.js"
    )
    content = _read(file_path)
    ...
    assert (
        "const measureRoot = blockEl.querySelector('.caderno-text-editor');" in content
    )
    assert "measureRoot ? measureRoot.scrollHeight : 0" in content
    assert (
        "measureRoot ? measureRoot.getBoundingClientRect().height : blockEl.getBoundingClientRect().height"
        in content
    )
...
def test_caderno_slash_menu_anchors_to_caret_and_hides_empty_toolbar():
    ...
    assert (
        ".caderno-block.is-active:not(.is-editor-empty):not(.is-slash-trigger):not(.is-slash-menu-open) .caderno-block-resize-handle"
        in css_content
    )
...
def test_caderno_composer_toolbar_is_the_primary_add_flow():
    ...
    js_content = _read(js_path)
    html_path = (
        Path(__file__).resolve().parents[2] / "templates" / "caderno" / "index.html"
    )
...
def test_caderno_reference_cards_drag_from_surface_and_link_only_on_title():
    ...
    assert "white-space: normal;" in _selector_body(
        css_content, ".caderno-ref-card-title-link"
    )
    assert "align-self: flex-start;" in _selector_body(
        css_content, ".caderno-ref-card-title-link"
    )
    assert "cursor: grab;" in _selector_body(css_content, ".caderno-ref-card")
    assert "cursor: pointer;" in _selector_body(
        css_content, ".caderno-ref-card-title-link"
    )
...
def test_caderno_click_outside_clears_selection_and_delete_uses_hover():
    ...
    assert (
        ".caderno-block.is-active:not(.is-editor-empty):not(.is-slash-trigger):not(.is-slash-menu-open) .caderno-block-toolbar-delete"
        not in css_content
    )
...
def test_caderno_dragging_parent_keeps_attached_notes_as_visual_companions():
    ...
    assert (
        "function buildDragChildCompanions(parentBlockId, parentRect) {" in js_content
    )

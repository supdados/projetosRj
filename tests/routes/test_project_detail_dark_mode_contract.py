...
def test_project_stage_task_modal_dark_mode_uses_single_surface_base():
    ...
    scoped_css = _read(
        root / "static" / "css" / "projects" / "detail" / "stage-task-modal-scoped.css"
    )
...
# test_project_header_meta_chips_render_with_scoped_classes removido na migração
# SPA: fazia GET /project/<id> (agora redirect 302). Os chips do header são da
# tela SPA /projetos/<id> (testes Svelte/contrato da API).

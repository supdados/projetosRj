...
def test_admin_edit_user_hides_cpf_and_sub_when_govbr_is_linked(
    ...
    response = client_admin.get(
        f"/admin/users/edit/{seed_data['editable_user_id']}", follow_redirects=False
    )
...
def test_admin_edit_linked_user_preserves_cpf_and_sub_when_fields_are_hidden(
    app, client_admin, seed_data
):
    with app.app_context():
        user = db.session.get(User, seed_data["editable_user_id"])
...
def test_admin_own_profile_hides_cpf_and_sub_when_govbr_is_linked(
    app, client_admin, seed_data
):
    with app.app_context():
        admin = db.session.get(User, seed_data["admin_id"])
    ...
    response = client_admin.get(
        f"/admin/users/edit/{seed_data['admin_id']}", follow_redirects=False
    )
...
def test_admin_edit_user_does_not_render_sub_field_when_unlinked(

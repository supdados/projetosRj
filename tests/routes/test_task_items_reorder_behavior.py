...
def test_reorder_tasks_hub_applies_mixed_payload_per_scope(app, client_user, seed_data):
    with app.app_context():
        ...
        foreign_project_before = db.session.get(TaskItem, seed_data['foreign_task_id']).ordem
    ...
    with app.app_context():
        ...
        assert db.session.get(TaskItem, seed_data['foreign_task_id']).ordem == foreign_project_before
        assert db.session.get(TaskItem, foreign_orphan_id).ordem == foreign_orphan_before


def test_reorder_tasks_hub_admin_reorders_all_orphans_in_single_scope(app, client_admin, seed_data):
    with app.app_context():

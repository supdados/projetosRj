    ...
    with app.app_context():
        notification_for_b = (
            UserNotification.query.filter_by(
                recipient_user_id=user_b_id, event_type="task_comment_added"
            )
...
def test_assignment_change_notifies_assigned_user(app, seed_data):
    with app.app_context():
        actor = _create_user(
            "notif_actor_3", "Notif Actor 3", areas=["Auditoria"], is_admin=True
        )
        assignee = _create_user(
            "notif_assignee_1", "Notif Assignee 1", areas=["Auditoria"]
        )
        actor_id = actor.id
    ...
    with app.app_context():
        notification = (
            UserNotification.query.filter_by(
                recipient_user_id=assignee_id, event_type="task_assignment"
            )
            .order_by(UserNotification.id.desc())
            .first()
        )
...
def test_legacy_project_without_owner_notifies_area_admin(app):
    with app.app_context():
        area_admin = _create_user(
            "area_admin_1", "Area Admin 1", areas=["CHEGAB"], is_admin=True
        )

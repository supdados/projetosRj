...
def test_setup_db_json_returns_status_details_and_sync_summary(app, client):
    response = client.get('/setup_db', headers={'Accept': 'application/json'})
    ...
    assert payload['detalhes']['banco_de_dados'].startswith('sqlite:///')
...
def test_setup_db_force_true_keeps_schema_available_and_returns_html(app, client):
    response = client.get('/setup_db?force=true')

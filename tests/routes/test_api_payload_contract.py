...
def test_api_goal_endpoints_return_404_with_error_payload_for_invalid_ids(client_user):
    ...
    assert indicadores_response.get_json() == {'error': 'Resultado esperado não encontrado'}


def test_api_templates_and_template_stages_return_sorted_shape(app, client_user, seed_data):
    with app.app_context():
        ...
        db.session.add_all(
            [
                StageTemplateItem(name='Descoberta', duration_days=5, order=0, templateId=template.id),
                StageTemplateItem(name='Entrega', duration_days=2, order=1, templateId=template.id),
            ]
        )
        db.session.commit()
    ...
    assert any(item == {'id': seed_data['template_id'], 'name': 'Template Base'} for item in templates_payload)

...
def _patch_token(monkeypatch, token='access-abc'):
    monkeypatch.setattr(
        calendar_sync, 'ensure_google_access_token',
...
def test_sync_creates_when_no_google_event_id(monkeypatch):
    ...
    def _fake_create(config, *, access_token, calendar_id, event_payload, conference_data_version):
...
def test_sync_without_conference_sets_version_zero(monkeypatch):
    monkeypatch.setattr(calendar_sync, 'utc_now', lambda: datetime.datetime(2026, 1, 15))
    _patch_token(monkeypatch)
    ...
    sync_local_event_to_google({}, _local_event(), _connection(), create_conference=False)
...
def test_sync_update_success_updates_local_fields(monkeypatch):
    ...
    monkeypatch.setattr(
        calendar_sync, 'update_google_calendar_event',
    ...
    monkeypatch.setattr(calendar_sync, 'create_google_calendar_event', _should_not_create)
...
def test_sync_update_404_falls_back_to_create(monkeypatch):
    ...
    monkeypatch.setattr(
        calendar_sync, 'create_google_calendar_event',
...
def test_delete_remote_event_returns_false_when_no_event_id():
    connection = _connection()
    assert calendar_sync.delete_remote_event({}, connection, google_event_id='') is False
...
def test_delete_remote_event_calls_api(monkeypatch):
    ...
    out = calendar_sync.delete_remote_event(
        {}, _connection(), google_event_id='evt-1', google_calendar_id='cal-x',
    )
    assert out is True

...
def test_creation_without_template_does_not_register_usage(app, client_admin, seed_data):
...
def test_creation_with_invalid_template_id_does_not_register(app, client_admin, seed_data):
    response = client_admin.post(
        '/add_project',
        data={
...
def test_import_model_after_creation_registers_post_import(app, client_admin, seed_data):
...
def test_distinct_project_count_matches_expectation(app, client_admin, seed_data):
    ...
    with app.app_context():
        distinct_projects = db.session.query(
            func.count(func.distinct(StageTemplateUsage.project_id))
        ).filter(StageTemplateUsage.template_id == template_id).scalar()

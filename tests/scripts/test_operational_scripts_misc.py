...
def _load_migration_unificacao_module():
    ...
    if spec is None or spec.loader is None:
        raise AssertionError('Não foi possível carregar migration_unificacao_tarefas para teste.')
...
def test_migrate_production_build_engine_prefers_database_url(monkeypatch):
    ...
    monkeypatch.setenv('DATABASE_URL', 'mysql+pymysql://user:secret@db.example.com/projetos')
...
def test_migrate_production_build_engine_builds_uri_from_env(monkeypatch):
    ...
    assert engine.uri == 'mysql+pymysql://db_user:senha+com+espaco@mysql.internal:3307/projetos'

...
def test_sync_objectives_catalog_main_supports_dry_run(app, monkeypatch, capsys):
    ...
    def fake_sync_goal_catalog_to_db(*, commit):
        ...
        return {'objetivos_upserted': 3, 'resultados_upserted': 7, 'indicadores_upserted': 11}

    monkeypatch.setattr(sync_objectives_catalog.db, 'create_all', fake_create_all)
    monkeypatch.setattr(sync_objectives_catalog, 'sync_goal_catalog_to_db', fake_sync_goal_catalog_to_db)
...
def test_sync_objectives_catalog_main_returns_error_code_on_failure(app, monkeypatch, capsys):
    monkeypatch.setattr(sync_objectives_catalog, 'app', app)
    monkeypatch.setattr(
        sync_objectives_catalog,
        'parse_args',

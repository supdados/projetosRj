...
def test_create_user_notifications_deduplicates_discards_actor_and_ignores_unknown_ids(app):
    with app.app_context():
        ...
        created_count = create_user_notifications(
            [recipient_b.id, recipient_a.id, recipient_a.id, str(actor.id), 999999, None],
        ...
        notifications = UserNotification.query.order_by(UserNotification.recipient_user_id.asc()).all()
        assert [notification.recipient_user_id for notification in notifications] == [
            recipient_a.id,
            recipient_b.id,
        ]
        assert all(notification.actor_user_id == actor.id for notification in notifications)
        assert all(notification.event_type == 'unit_event' for notification in notifications)
...
def test_resolve_project_owner_without_history_falls_back_to_area_admins(app):
    with app.app_context():
        ...
        _create_user('other_admin_without_history', 'Other Admin Without History', is_admin=True, areas=['VPD'])
...
def test_resolve_task_collaborator_user_ids_deduplicates_creator_responsaveis_and_commenters(app):
    with app.app_context():
        ...
        comment_author = _create_user('comment_author', 'Comment Author', areas=['Auditoria'])
        ...
        db.session.add_all(
            [
                TaskComment(content='Comentario do criador', user_id=creator.id, task_id=task.id),
                TaskComment(content='Comentario externo', user_id=comment_author.id, task_id=task.id),
                TaskComment(content='Comentario externo 2', user_id=comment_author.id, task_id=task.id),
            ]
        )
        db.session.commit()

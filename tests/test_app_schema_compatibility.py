...
def test_schema_compatibility_upgrades_legacy_caderno_layout(tmp_path):
    isolated_app = _create_isolated_app(tmp_path, "legacy-caderno-schema.sqlite")
    ...
    with isolated_app.app_context():
        ...
        db.session.execute(text("""
                INSERT INTO user (id, username, password_hash, name, orgao, is_admin)
                VALUES (1, 'legacy-caderno', 'hash', 'Legacy Caderno', 'Orgao Legacy', 0)
                """))
        db.session.execute(text("""
                CREATE TABLE caderno_block (
                    id INTEGER PRIMARY KEY,
                ...
                """))
        db.session.execute(text("""
                INSERT INTO caderno_block (id, user_id, block_type, content, position)
                VALUES
                    (1, 1, 'text', 'Bloco antigo', 1000),
                    (2, 1, 'project', '', 2000)
                """))
        db.session.commit()
        ...
        inspector = inspect(db.engine)
        caderno_columns = {
            column["name"] for column in inspector.get_columns("caderno_block")
        }
        ...
        blocks = db.session.execute(text("""

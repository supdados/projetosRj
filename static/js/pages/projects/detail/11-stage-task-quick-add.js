(function () {
    ...
    function loadPanel(options) {
        ...
        const url = fetchApi.buildStageTasksUrl(stageTasksUrlTemplate, currentEtapaId);
        return fetchApi.fetchStageTasks(url, requestController)
            .then((payload) => {
                if (contentHost) {
                    contentHost.innerHTML = payload.html || '';
                }
                resetFields();
                rows.highlightCreatedRow(contentHost, opts.highlightTaskId);
                if (opts.focusAdd) {
                    showForm();
                } else {
                    focusInitialControl();
    ...
    function submitTask() {
        ...
        fetchApi.createTask(addTaskUrl, config, {
            ...
            .then((payload) => {
                ...
                return loadPanel({ focusAdd: true, highlightTaskId: payload && payload.task ? payload.task.id : null });
            })
            .catch((error) => {
    ...
    function open(trigger) {
        ...
        currentEtapaId = trigger.getAttribute('data-etapa-id');
        currentProjectId = trigger.getAttribute('data-project-id') || config.projectId || currentProjectId;
        responsavelNames = [];
        if (stageTitleEl) {
            stageTitleEl.textContent = trigger.getAttribute('data-etapa-descricao') || 'Etapa';
        }

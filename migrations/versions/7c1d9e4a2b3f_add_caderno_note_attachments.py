...
def upgrade():
    ...
    with op.batch_alter_table('caderno_block', schema=None) as batch_op:
        if not _column_exists(inspector, 'caderno_block', 'attached_to_block_id'):
            batch_op.add_column(sa.Column('attached_to_block_id', sa.Integer(), nullable=True))
        if not _column_exists(inspector, 'caderno_block', 'attached_offset_x'):
            batch_op.add_column(sa.Column('attached_offset_x', sa.Integer(), nullable=False, server_default='0'))
        if not _column_exists(inspector, 'caderno_block', 'attached_offset_y'):
            batch_op.add_column(sa.Column('attached_offset_y', sa.Integer(), nullable=False, server_default='0'))

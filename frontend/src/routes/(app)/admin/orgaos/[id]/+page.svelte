<script lang="ts">
	/**
	 * Form "Editar órgão" (Admin > Órgãos). URL `/spa/admin/orgaos/[id]`. Carrega
	 * o órgão via `GET /api/admin/orgaos/<id>` (que já traz `is_root`, os
	 * candidatos a pai — excluindo ele e seus descendentes — e os tipos) e salva
	 * via `POST /api/admin/orgaos/<id>`. O `id` vem do parâmetro de rota.
	 *
	 * Em modo raiz o pai não é editável. Validações (tipo vs. filhos, movimentação
	 * de pai, profundidade) são do backend: 422/409 viram mensagem no topo;
	 * `not_found` (404) é tratado à parte. Sucesso navega de volta para a árvore.
	 * Links base-aware (`$app/paths`).
	 *
	 * Referência visual: templates/admin/orgao_form.html.
	 */
	import { onMount } from 'svelte';
	import { base } from '$app/paths';
	import { goto } from '$app/navigation';
	import { page } from '$app/stores';
	import { fetchOrgaoDetail, updateOrgao } from '$lib/api/adminOrgaos';
	import { ApiClientError } from '$lib/api/client';
	import type { CandidatoPai, OrgaoTipo } from '$lib/types/adminOrgaos';
	import OrgaoFormFields from '$lib/components/OrgaoFormFields.svelte';

	type LoadState = 'loading' | 'ready' | 'error';

	const orgaoId = $derived(Number($page.params.id));

	let loadState = $state<LoadState>('loading');
	let loadError = $state<string>('');
	let errorKind = $state<'forbidden' | 'not_found' | 'generic'>('generic');

	let tipos = $state<OrgaoTipo[]>([]);
	let candidatosPai = $state<CandidatoPai[]>([]);
	let isRoot = $state(false);

	let nome = $state('');
	let sigla = $state('');
	let tipoId = $state('');
	let paiId = $state('');
	let ordem = $state('0');
	let ativo = $state(true);
	let codigoExterno = $state('');
	let dataInicio = $state('');
	let dataFim = $state('');

	let submitting = $state(false);
	let submitError = $state('');

	let inFlight: AbortController | null = null;

	async function load(): Promise<void> {
		loadState = 'loading';
		loadError = '';
		errorKind = 'generic';
		inFlight?.abort();
		const controller = new AbortController();
		inFlight = controller;
		try {
			const data = await fetchOrgaoDetail(orgaoId, controller.signal);
			if (controller.signal.aborted) return;
			tipos = data.tipos;
			candidatosPai = data.candidatos_pai;
			isRoot = data.is_root;
			const o = data.orgao;
			nome = o.nome ?? '';
			sigla = o.sigla ?? '';
			tipoId = o.tipo_id !== null ? String(o.tipo_id) : '';
			paiId = o.pai_id !== null ? String(o.pai_id) : '';
			ordem = o.ordem !== null ? String(o.ordem) : '0';
			ativo = o.ativo;
			codigoExterno = o.codigo_externo ?? '';
			dataInicio = o.data_inicio_vigencia ?? '';
			dataFim = o.data_fim_vigencia ?? '';
			loadState = 'ready';
		} catch (err) {
			if (controller.signal.aborted) return;
			if (err instanceof ApiClientError && err.code === 'unauthenticated') return;
			if (err instanceof ApiClientError && err.code === 'forbidden') {
				errorKind = 'forbidden';
				loadError = 'Você não tem permissão para gerenciar órgãos.';
			} else if (err instanceof ApiClientError && err.code === 'not_found') {
				errorKind = 'not_found';
				loadError = 'Órgão não encontrado.';
			} else {
				loadError = err instanceof Error ? err.message : 'Falha ao carregar o órgão.';
			}
			loadState = 'error';
		}
	}

	async function onSubmit(event: SubmitEvent): Promise<void> {
		event.preventDefault();
		if (submitting) return;
		submitting = true;
		submitError = '';
		try {
			await updateOrgao(orgaoId, {
				nome,
				sigla,
				tipo_id: tipoId,
				pai_id: isRoot ? null : paiId,
				ordem,
				ativo: ativo ? '1' : '0',
				codigo_externo: codigoExterno,
				data_inicio_vigencia: dataInicio,
				data_fim_vigencia: dataFim
			});
			await goto(`${base}/admin/orgaos`);
		} catch (err) {
			if (err instanceof ApiClientError && err.code === 'unauthenticated') return;
			submitError = err instanceof Error ? err.message : 'Não foi possível salvar o órgão.';
		} finally {
			submitting = false;
		}
	}

	onMount(() => {
		void load();
		return () => inFlight?.abort();
	});
</script>

<svelte:head>
	<title>Editar órgão — ProjetosRJ</title>
</svelte:head>

<section aria-labelledby="editar-orgao-title" class="flex flex-col gap-6">
	<header class="flex flex-col gap-2">
		<a
			href={`${base}/admin/orgaos`}
			class="w-fit text-sm text-text-secondary hover:text-text-primary hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
		>
			← Voltar à árvore
		</a>
		<h1 id="editar-orgao-title" class="font-heading text-2xl font-bold text-text-primary">
			Editar órgão
		</h1>
	</header>

	{#if loadState === 'loading'}
		<p role="status" aria-live="polite" class="text-text-secondary">Carregando órgão…</p>
	{:else if loadState === 'error'}
		<div role="alert" class="flex flex-col items-start gap-3 rounded-lg border border-danger bg-surface px-5 py-4">
			<p class="text-text-primary">{loadError}</p>
			{#if errorKind === 'generic'}
				<button
					type="button"
					onclick={() => load()}
					class="rounded-md border border-border-subtle bg-surface px-4 py-2 text-sm font-medium text-text-primary hover:bg-surface-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
				>
					Tentar novamente
				</button>
			{:else}
				<a
					href={`${base}/admin/orgaos`}
					class="rounded-md border border-border-subtle bg-surface px-4 py-2 text-sm font-medium text-text-primary hover:bg-surface-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
				>
					Voltar à árvore
				</a>
			{/if}
		</div>
	{:else}
		<form class="flex flex-col gap-5" onsubmit={onSubmit}>
			{#if submitError}
				<div role="alert" class="rounded-md border border-danger bg-surface px-4 py-3 text-sm text-text-primary">
					{submitError}
				</div>
			{/if}

			<OrgaoFormFields
				{tipos}
				{candidatosPai}
				{isRoot}
				bind:nome
				bind:sigla
				bind:tipoId
				bind:paiId
				bind:ordem
				bind:ativo
				bind:codigoExterno
				bind:dataInicio
				bind:dataFim
			/>

			<div class="flex items-center gap-3">
				<button
					type="submit"
					disabled={submitting}
					class="inline-flex h-9 items-center rounded-md bg-primary-600 px-3.5 text-sm font-semibold text-white shadow-sm transition-all duration-fast hover:bg-primary-700 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none"
				>
					{submitting ? 'Salvando…' : 'Salvar alterações'}
				</button>
				<a
					href={`${base}/admin/orgaos`}
					class="rounded-md border border-border-subtle bg-surface px-5 py-2 text-sm font-medium text-text-primary hover:bg-surface-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500"
				>
					Cancelar
				</a>
			</div>
		</form>
	{/if}
</section>

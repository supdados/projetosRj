<script lang="ts">
	...
	const inputClass =
		'rounded-md border border-border-subtle bg-surface px-3 py-2 text-sm text-text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500';
	const labelClass = 'text-xs font-semibold uppercase tracking-wide text-text-muted';
</script>

<div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
	<div class="flex flex-col gap-1 sm:col-span-2">
		<label for="orgao-nome" class={labelClass}>Nome completo <span class="text-danger">*</span></label>
		<input id="orgao-nome" type="text" required maxlength="255" bind:value={nome} class={inputClass} />
	</div>

	<div class="flex flex-col gap-1">
		<label for="orgao-sigla" class={labelClass}>Sigla <span class="text-danger">*</span></label>
		<input id="orgao-sigla" type="text" required maxlength="50" bind:value={sigla} class={inputClass} />
	</div>
	...
	<div class="flex flex-col gap-1 sm:col-span-2">
		<span class={labelClass}>Órgão pai {#if !isRoot}<span class="text-danger">*</span>{/if}</span>
		{#if isRoot}
			<input type="text" value="— raiz da hierarquia —" disabled class="{inputClass} opacity-70" />
		{:else}
	...
	<div class="flex flex-col gap-1">
		...
		<input id="orgao-codigo" type="text" bind:value={codigoExterno} class={inputClass} />
	</div>

	<div class="flex flex-col gap-1">
		<label for="orgao-ordem" class={labelClass}>Ordem entre irmãos</label>
		<input id="orgao-ordem" type="number" bind:value={ordem} class={inputClass} />
	</div>
	...
	<div class="flex items-center gap-2 sm:col-span-2">
		<input id="orgao-ativo" type="checkbox" bind:checked={ativo} class="h-4 w-4 rounded border-border-subtle text-primary-700 focus:ring-primary-500" />

...
def ensure_caderno_schema(emit_output=True):
    ...
    try:
        ...
        if 'caderno_state' not in table_names_before and _table_exists(inspector, 'caderno_state'):
            changes.append('caderno_state.created')

        if not _table_exists(inspector, 'caderno_block'):
            _emit('   ✓ Estruturas do caderno criadas sem blocos legados para migrar.', emit_output)
            return {'success': True, 'changes': changes}
        ...
        if rows and should_backfill_layout:
            ...
            for row in rows:
                ...
                _occupy_caderno_cells(user_occupied, grid_x, grid_y, layout['grid_w'], layout['grid_h'])
                db.session.execute(
                    text(
                        """

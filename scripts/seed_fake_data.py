    ...
    *,
    ...
    with app.app_context():
        ...
        for project_index in range(projects):
            ...
            abep_indicator = ABEP_INDICADORES_OPTIONS[project_index % len(ABEP_INDICADORES_OPTIONS)]['value']
            ...
            for indicador_id in indicador_ids:
                db.session.add(IndicadorProjeto(project_id=project.id, indicador_id=indicador_id))
...
def parse_args(argv):
    parser = argparse.ArgumentParser(description='Popula o banco com dados fake para testes.')
    ...
    parser.add_argument('--no-reset', action='store_true', help='Nao reseta tabelas antes de popular.')
    parser.add_argument('--yes', action='store_true', help='Confirma execucao sem prompt.')

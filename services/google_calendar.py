...
def _resolve_client_secret_path(config):
    configured = str(
        config.get("GOOGLE_CALENDAR_CLIENT_SECRET_FILE", "config/client_secret.json")
    ).strip()
    if not configured:
...
def load_google_oauth_client(config):
    ...
    try:
        ...
        raise GoogleCalendarError(
            "Arquivo de credenciais Google inválido (JSON)."
        ) from exc

    client = payload.get("web") or payload.get("installed")
    if not isinstance(client, dict):
        raise GoogleCalendarError(
            'Credencial Google inválida: esperado bloco "web" ou "installed".'
        )
...
def is_google_calendar_enabled(config):
    if str(config.get("GOOGLE_CALENDAR_ENABLED", "true")).strip().lower() in {
        "0",
        "false",
        "no",
        "off",
    ...
    return bool(
        ...
        and str(client.get("client_secret", "")).strip()
    )
...
def get_google_calendar_settings(config, *, redirect_uri=None):
    if not is_google_calendar_enabled(config):
        raise GoogleCalendarError(
            "Integração Google Calendar não está habilitada ou configurada."
        )
    ...
    resolved_redirect_uri = str(
        redirect_uri
        or configured_redirect_uri
        or (redirect_uris[0] if redirect_uris else "")
    ).strip()
    if not resolved_redirect_uri:
        raise GoogleCalendarError(
            "Nenhuma redirect_uri disponível para OAuth do Google Calendar."
        )
    ...
    scope = (
        str(config.get("GOOGLE_CALENDAR_SCOPE", GOOGLE_CALENDAR_SCOPE_EVENTS)).strip()
        or GOOGLE_CALENDAR_SCOPE_EVENTS
    )
    ...
    return GoogleCalendarSettings(
        ...
        timeout_seconds=_normalize_timeout(
            config.get("GOOGLE_CALENDAR_TIMEOUT_SECONDS", 10)
        ),
    )


def _http_json_request(
    *, method, url, timeout_seconds, headers=None, form_data=None, json_data=None
):
    ...
    if form_data is not None and json_data is not None:
        raise GoogleCalendarError(
            "Não é permitido enviar form_data e json_data na mesma requisição."
        )
    ...
    try:
        ...
        raise GoogleCalendarError(
            f"Falha de conectividade com Google APIs: {exc}"
        ) from exc
    except Exception as exc:
        raise GoogleCalendarError(
            f"Falha inesperada na chamada para Google APIs: {exc}"
        ) from exc
    ...
    try:
        ...
        raise GoogleCalendarError(
            "Resposta da API Google não está em JSON válido."
        ) from exc
...
def get_google_userinfo(config, *, access_token):
    ...
    if "sub" not in payload:
        raise GoogleCalendarError(
            "Resposta do userinfo sem identificador da conta Google."
        )
    return payload
...
def create_google_calendar_event(
    config, *, access_token, calendar_id, event_payload, conference_data_version=0
):
    settings = get_google_calendar_settings(config)
